...
void extract(size_t i, path arc, path to){
	fs::remove_all(to);
...
void test()
{
	path hf = getenv("HOME");
	ASSERT(!hf.empty());
	path atest_src = hf / "atest-src";
	path atest_tmp = hf / "atest-tmp";
	path atest_add = hf / "atest-add";
	path atest_rmv = hf / "atest-rmv";
	path atest_arc = hf / "atest-arc";
