...
{
	return fmt::format("{:%Y %B %d %H:%M:%S}", chrono::time_point_cast<chrono::seconds>(to_sys_clock(time)));
}
...
int run(int argc, const char *argv[]){
	...
	auto report_warning = [&return_code](std::string &&h, std::string &&w){
		cfmt::cprint(stderr, "{fr}{}{fd}", h);
		w.insert(0, "\n");
		find_and_replace(w, "\n", "\n  ");
		w += '\n';
		cfmt::cprint(stderr, w);
		fflush(stderr);
		return_code = 1;
	};
	auto report_progress = [](uint progress_in_permil){
		cfmt::cprint("{}.{}%\n", progress_in_permil/10, progress_in_permil % 10);
		cfmt::move_cursor_up();
	};
	...
	if (cmd_line.command() == "version"){
		...
		cfmt::cprint(stderr, tr_txt("{fr}unknown command {}{fd}\n"), cmd_line.command());
		return 1;
	}
	return return_code;
}
